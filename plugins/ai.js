  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        const apiUrl = `https://widipe.com/removebg?url=${encodeURIComponent(url)}`;
  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        const apiUrl = `https://widipe.com/stablediffusion?text=${encodeURIComponent(text)}`;
        const response = await axios.get(apiUrl, {
          headers: {
            "accept": "application/json",
          },
  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        const apiUrl = `https://widipe.com/bard?text=${encodeURIComponent(text)}`;
        const response = await axios.get(apiUrl, {
          headers: {
            "accept": "application/json",
          },
        ...
        return await message.send(`*BYTE-MD ʙᴀʀᴅ ᴀɪ:*\n ${result}`, { quoted: message });
      } catch (error) {
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("BYTE-MD• ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            _0x2dfb59[0].includes("BYTE-MD • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
